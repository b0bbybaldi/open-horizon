...
log.warning()
{
  if [ "${DEBUG:-}" = true ]; then echo "+++ WARN -- $0 $$ -- ${1}" > /dev/stderr; fi
}
...
if [ -s "${APACHE_CONF}" ]; then
    ...
    # edit defaults
    log.debug "Changing Listen to ${APACHE_PORT}"
    sed -i 's|^Listen\(.*\)|Listen '${APACHE_PORT}'|' "${APACHE_CONF}"
    log.debug "Changing ServerAdmin to ${APACHE_ADMIN}"
    ...
    log.debug "Starting apache2"
    # start HTTP daemon 
    service apache2 start
else
    log.warning "Did not find Apache configuration at ${APACHE_CONF}"
fi
